...
protein_distribution_plot <- pilot_data %>%
  ...
  geom_col() + 
  scale_fill_brewer(palette="Set3",labels=c("Beef","Chicken","Fish","Legumes","Pork","Tofu"),name="") +
...
annual_impact_plot <- impact_data %>%
  ...
  geom_col() + 
  scale_fill_manual(values=c("royalblue4","mediumseagreen"),name="",labels=c("Reductions in methane","Reductions in GHG emissions, excluding methane")) +
  scale_x_continuous(labels=scales::comma) +
  ylab("") + 
